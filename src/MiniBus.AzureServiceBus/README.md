...
The Billing Function App sample under `samples/MiniBus.Samples.FunctionApp` includes a repo-owned Azure Service Bus emulator topology and uses the same `ServiceBusClient` plus `AzureServiceBusSender` registration locally. That sample is the runnable local transport reference path; live Azure Service Bus coverage remains separate.
